...
const TextType = ({
  ...
  useEffect(() => {
    ...
    const executeTypingAnimation = () => {
      if (isDeleting) {
        if (displayedText === "") {
          ...
          timeout = setTimeout(executeTypingAnimation, 10);
        } else {
          timeout = setTimeout(() => {
    ...
    // For the first text that's already displayed, wait for initialDelay before starting to delete
    if (currentCharIndex === processedText.length && !isDeleting && currentTextIndex === 0 && displayedText === processedText) {
      timeout = setTimeout(() => {
        setIsDeleting(true);
      }, initialDelay);
    } else if (currentCharIndex === 0 && !isDeleting && displayedText === "") {
      // When text is fully deleted, start typing immediately (handled by the 1ms timeout in deletion logic)
